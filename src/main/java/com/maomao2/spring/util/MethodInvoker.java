...
package com.maomao2.spring.util;
 ...
 * optionally specify arguments, prepare the invoker. Afterwards, you may invoke the method any number of times,
 * obtaining the invocation result.
 *
...
public class MethodInvoker {
  ...
  /**
   * Set arguments for the method invocation. If this property is not set, or the Object array is of length 0, a method
   ...
   * arguments. A direct match, i.e. type Integer -> arg of class Integer, does not increase the result - all direct
   * matches means weight 0. A match between type Object and arg of class Integer would increase the weight by 2, due to
   * the superclass 2 steps up in the hierarchy (i.e. Object) being the last one that still matches the required type
   * Object. Type Number and class Integer would increase the weight by 1 accordingly, due to the superclass 1 step up
   * the hierarchy (i.e. Number) still matching the required type Number. Therefore, with an arg of type Integer, a
   * constructor (Integer) would be preferred to a constructor (Number) which would in turn be preferred to a
   * constructor (Object). All argument weights get accumulated.
   * <p>Note: This is the algorithm used by MethodInvoker itself and also the algorithm
   * used for constructor and factory method selection in Spring's bean container (in case of lenient constructor
   * resolution which is the default for regular bean definitions).
   *
